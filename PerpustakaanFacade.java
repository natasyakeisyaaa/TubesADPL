...
/**
 * FACADE : PerpustakaanFacade
 * Menyederhanakan akses ke 4 subsistem: LayananBuku, LayananAnggota, LayananPeminjaman, LayananPencarian
 */
public class PerpustakaanFacade {

    //singleton
    private static PerpustakaanFacade instance;
    ...
    public static PerpustakaanFacade getInstance() {
        if (instance == null) {
            instance = new PerpustakaanFacade();
        }
        return instance;
    }
    ...
    public void tampilkanSemuaBuku() {
        ...
        if (semua.isEmpty()) { System.out.println("  (Koleksi kosong)"); return; }
    ...
    //MANAJEMEN ANGGOTA
    public void daftarAnggota(String id, String nama, String tglLahir, String noTelp, String alamat) {
        if (layananAnggota.isIdTerdaftar(id)) {
            System.out.println("[Facade] ID anggota sudah terdaftar: " + id);
            return;
        }
        layananAnggota.daftarAnggota(new Anggota(id, nama, tglLahir, noTelp, alamat));
    ...
    //PENCARIAN
    public void cariBukuByJudul(String query) {
        List<Buku> hasil = layananPencarian.cariByJudul(layananBuku.getSemuaBuku(), query);
        
        tampilkanHasilCari("Judul mengandung '" + query + "'", hasil);
    }
